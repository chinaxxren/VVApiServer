...
@interface MultipartFormDataParser : NSObject {
    ...
    MultipartMessageHeader *_currentHeader;

    BOOL _waitingForCRLF;
    BOOL _reachedEpilogue;
    BOOL _processedPreamble;
    BOOL _checkForContentEnd;

#if __has_feature(objc_arc_weak)
    __weak id <MultipartFormDataParserDelegate> _delegate;
#else
    __unsafe_unretained id<MultipartFormDataParserDelegate> _delegate;
#endif
    int _currentEncoding;
    NSStringEncoding _formEncoding;
}
...
#if __has_feature(objc_arc_weak)
@property(weak, readwrite) id _delegate;
#else
