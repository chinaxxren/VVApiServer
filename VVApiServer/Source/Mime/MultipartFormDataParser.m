...
@synthesize _delegate, formEncoding = _formEncoding;
...
- (BOOL)appendData:(NSData *)data {
    ...
    if (nil == _boundaryData) {
        VVHTTPLogError(@"MultipartFormDataParser: Trying to parse multipart without specifying a valid boundary");
        assert(false);
        return NO;
    }
    ...
    if (_pendingData.length) {
        [_pendingData appendData:data];
        workingData = _pendingData;
    }
    ...
    NSUInteger sizeToLeavePending = _boundaryData.length;

    if (!_reachedEpilogue && workingData.length <= sizeToLeavePending) {
        // not enough data even to start parsing.
        // save to pending data.
        if (!_pendingData.length) {
            [_pendingData appendData:data];
        }
        if (_checkForContentEnd) {
            if (_pendingData.length >= 2) {
                if (*(uint16_t *) (_pendingData.bytes + offset) == 0x2D2D) {
                    // we found the multipart end. all coming next is an epilogue.
                    VVHTTPLogVerbose(@"MultipartFormDataParser: End of multipart message");
                    _waitingForCRLF = YES;
                    _reachedEpilogue = YES;
                    offset += 2;
                } else {
                    _checkForContentEnd = NO;
                    _waitingForCRLF = YES;
                    return YES;
                }
            } else {
    ...
    while (true) {
        if (_checkForContentEnd) {
            ...
            if (offset < workingData.length - 1) {
                ...
                if (offset < workingData.length) {
                    [_pendingData setData:[NSData dataWithBytes:workingData.bytes + workingData.length - 1 length:1]];
                } else {
                    // there is no unhandled data now, wait for more chunks
                    [_pendingData setData:[NSData data]];
                }
                return YES;
            }
        }
        ...
        if (_reachedEpilogue) {
            ...
            if ([_delegate respondsToSelector:@selector(processEpilogueData:)]) {
                ...
                [_delegate processEpilogueData:epilogueData];
            }
            return YES;
        }

        if (nil == _currentHeader) {
            ...
            if (-1 == headerEnd) {
                ...
                if (!_pendingData.length) {
                    ...
                    [_pendingData appendBytes:data.bytes + offset length:data.length - offset];
                } else {
                    if (offset) {
                        // save the current parse state; drop all handled data and save unhandled only.
                        _pendingData = [[NSMutableData alloc] initWithBytes:(char *) workingData.bytes + offset length:workingData.length - offset];
                    }
                }
                return YES;
            } else {
                ...
                _currentHeader = [[MultipartMessageHeader alloc] initWithData:headerData formEncoding:_formEncoding];

                if (nil == _currentHeader) {
                    // we've found the data is in wrong format.
                    VVHTTPLogError(@"MultipartFormDataParser: MultipartFormDataParser: wrong input format, coulnd't get a valid header");
                    return NO;
                }
                if ([_delegate respondsToSelector:@selector(processStartOfPartWithHeader:)]) {
                    [_delegate processStartOfPartWithHeader:_currentHeader];
                }

                VVHTTPLogVerbose(@"MultipartFormDataParser: MultipartFormDataParser: Retrieved part header.");
            }
        ...
        if (contentEnd == -1) {
            ...
            int leaveTrailing = [self numberOfBytesToLeavePendingWithData:data length:sizeToPass encoding:_currentEncoding];
            sizeToPass -= leaveTrailing;

            if (sizeToPass <= 0) {
                // wait for more data!
                if (offset) {
                    [_pendingData setData:[NSData dataWithBytes:(char *) workingData.bytes + offset length:workingData.length - offset]];
                }
                return YES;
            }
            // decode the chunk and let the delegate use it (store in a file, for example)
            NSData *decodedData = [MultipartFormDataParser decodedDataFromData:[NSData dataWithBytesNoCopy:(char *) workingData.bytes + offset length:workingData.length - offset - sizeToLeavePending freeWhenDone:NO] encoding:_currentEncoding];

            if ([_delegate respondsToSelector:@selector(processContent:WithHeader:)]) {
                VVHTTPLogVerbose(@"MultipartFormDataParser: Processed %"FMTNSINT" bytes of body", sizeToPass);

                [_delegate processContent:decodedData WithHeader:_currentHeader];
            }

            // store the unprocessed data till the next chunks come.
            [_pendingData setData:[NSData dataWithBytes:(char *) workingData.bytes + workingData.length - sizeToLeavePending length:sizeToLeavePending]];
            return YES;
        } else {
            ...
            if ([_delegate respondsToSelector:@selector(processContent:WithHeader:)]) {
                [_delegate processContent:[NSData dataWithBytesNoCopy:(char *) workingData.bytes + offset length:contentEnd - offset freeWhenDone:NO] WithHeader:_currentHeader];
            }

            if ([_delegate respondsToSelector:@selector(processEndOfPartWithHeader:)]) {
                [_delegate processEndOfPartWithHeader:_currentHeader];
                VVHTTPLogVerbose(@"MultipartFormDataParser: End of body part");
            }
            _currentHeader = nil;

            // set up offset to continue with the remaining data (if any)
            // cast to int because above comment suggests a small number
            offset = contentEnd + (int) _boundaryData.length;
            _checkForContentEnd = YES;
...
- (int)offsetTillNewlineSinceOffset:(int)offset inData:(NSData *)data {
    ...
    while (*(uint16_t *) (bytes + offset) != 0x0A0D) {
        ...
        if (!isspace(*(bytes + offset))) {
            VVHTTPLogWarn(@"MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line", *(bytes + offset));
        }
        if (!isspace(*(bytes + offset + 1))) {
            VVHTTPLogWarn(@"MultipartFormDataParser: Warning, non-whitespace character '%c' between boundary bytes and CRLF in boundary line", *(bytes + offset + 1));
        }
#endif
        offset++;
        if (offset >= length) {
...
- (int)processPreamble:(NSData *)data {
    ...
    char *boundaryBytes = (char *) _boundaryData.bytes + 2; // the first boundary won't have CRLF preceding.
    char *dataBytes = (char *) data.bytes;
    NSUInteger boundaryLength = _boundaryData.length - 2;
    ...
    if (offset == dataLength) {
        ...
        if (sizeToProcess > 0) {
            if ([_delegate respondsToSelector:@selector(processPreambleData:)]) {
                NSData *preambleData = [NSData dataWithBytesNoCopy:(char *) data.bytes length:data.length - offset - boundaryLength freeWhenDone:NO];
                [_delegate processPreambleData:preambleData];
                VVHTTPLogVerbose(@"MultipartFormDataParser: processed preamble");
            }
            _pendingData = [NSMutableData dataWithBytes:data.bytes + data.length - boundaryLength length:boundaryLength];
        }
        return -1;
    } else {
        if (offset && [_delegate respondsToSelector:@selector(processPreambleData:)]) {
...
//	http://tools.ietf.org/html/rfc2045#section-6.7
    ...
    while (count < length) {
        if (bytes[count] == '=') {
            ...
            if (length < 3) {
                VVHTTPLogWarn(@"MultipartFormDataParser: warning, trailing '=' in quoted printable data");
            }
            // soft newline
            if (bytes[0] == '\r') {
                bytes += 1;
        ...
        if ((unsigned char) bytes[count] > 126) {
            VVHTTPLogWarn(@"MultipartFormDataParser: Warning, character with code above 126 appears in quoted printable encoded data");
        }
#endif
