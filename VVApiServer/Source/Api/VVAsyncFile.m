...
**/
@interface VVAsyncFile ()

@property(nonatomic, weak) id <VVAsyncFileDelegate> delegate;

@end

@implementation VVAsyncFile
...
- (void)abort {
    ...
    if ([self.delegate respondsToSelector:@selector(responseDidAbort)]) {
        [self.delegate responseDidAbort];
    }
    aborted = YES;
}
...
- (void)processReadBuffer {
    ...
    if ([self.delegate respondsToSelector:@selector(responseHasAvailableData)]) {
        [self.delegate responseHasAvailableData];
    }
}
...
- (BOOL)openFileAndSetupReadSource {
    ...
    readQueue = dispatch_queue_create("vv.async.file", NULL);
