...
@interface VVHTTPConnection (PrivateAPI)
...
@implementation VVHTTPConnection
...
- (BOOL)supportsMethod:(NSString *)method atPath:(NSString *)path {
    ...
    //
    // For more information, you can always access the VVHTTPMessage request variable.
    //
    ...
    if ([VVWebSocket isWebSocketRequest:_requestMessage]) {
        ...
        if (ws == nil) {
            ...
            if ([asyncSocket delegate] == self) {
                ...
                // The VVWebSocket is using the socket,
...
- (void)sendResponseHeadersAndBody {
    ...
    // If a response is "chunked", this simply means the VVHTTPResponse object
...
- (void)continueSendingStandardResponseBody {
    ...
    // In the case of the asynchronous VVHTTPResponse, we don't want to blindly grab the new data,
    ...
    //
    // Note that this does not affect the rate at which the VVHTTPResponse object may generate data.
    // The VVHTTPResponse is free to do as it pleases, and this is up to the application's developer.
    // If the memory footprint is a concern, the developer creating the custom VVHTTPResponse object may freely
    // use the calls to readDataOfLength as an indication to start generating more data.
    // This provides an easy way for the VVHTTPResponse object to throttle its data allocation in step with the rate
...
- (NSObject <VVHTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path {
    ...
    if (filePath && [[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDir] && !isDir) {
        return [[VVHTTPFileResponse alloc] initWithFilePath:filePath forConnection:self];

        // Use me instead for asynchronous file IO.
        // Generally better for larger files.

        //	return [[[VVHTTPAsyncFileResponse alloc] initWithFilePath:filePath forConnection:self] autorelease];
    }
...
- (VVWebSocket *)webSocketForURI:(NSString *)path {
    ...
    // Then return an instance of your custom VVWebSocket here.
    //
    // For example:
    //
...
- (NSData *)preprocessResponse:(VVHTTPMessage *)message {
    ...
    [message setHeaderField:@"Date" value:now];

    // Add server capability headers
    [message setHeaderField:@"Accept-Ranges" value:@"bytes"];
    ...
    if (doneSendingResponse) {
        ...
        if (tag == HTTP_FINAL_RESPONSE) {
            ...
            if ([self shouldDie]) {
                ...
                // or if the socket gets taken over by someone else like a VVWebSocket.
...
- (void)responseHasAvailableData:(NSObject <VVHTTPResponse> *)sender {
    ...
    //
    // We do this to give the VVHTTPResponse classes the flexibility to call
